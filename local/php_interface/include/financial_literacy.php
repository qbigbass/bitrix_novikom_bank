<section class="section-layout bg-dark-10">
    <div class="container">
        <div class="row">
            <div class="col-12">
                <div class="card-promo card-promo--heavy-blue">
                    <div class="card-promo__wrapper d-flex flex-column h-100 row-gap-4 row-gap-md-6 row-gap-lg-7">
                        <div class="card-promo__header d-flex flex-column row-gap-3 row-gap-md-4">
                            <h2>Финансовая грамотность</h2>
                            <p class="text-l mb-0 w-md-60 w-lg-100">Финансовая грамотность позволит достичь гармонии с&nbsp;деньгами. <br>Они перестанут быть причиной переживаний или самоцелью и&nbsp;превратятся в&nbsp;инструмент достижения ваших целей. И&nbsp;вот каким будет путь к&nbsp;этому</p>
                        </div><img class="card-promo__image" src="img/big-illustrations/large-individual/faq-fin-competition.png" alt="" loading="lazy">
                        <div class="d-flex flex-column flex-md-row align-items-md-center gap-3 gap-md-6"><a class="btn btn-info btn-lg-lg w-100 w-md-auto" href="#">Подробнее</a>
                        </div>
                    </div>
                    <picture class="pattern-bg">
                        <source srcset="img/patterns/section-2/pattern-dark-s.svg" media="(max-width: 767px)">
                        <source srcset="img/patterns/section-2/pattern-dark-m.svg" media="(max-width: 1199px)"><img src="img/patterns/section-2/pattern-dark-l.svg" alt="bg pattern" loading="lazy">
                    </picture>
                </div>
            </div>
        </div>
    </div>
</section>
