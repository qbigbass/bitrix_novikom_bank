...
/**
 * Компонент основан на bitrix:main.map. Кастомные куски кода помечены как #CUSTOM#
 *
 * @var array $arParams
 * @var array $arResult
 * @global CUser $USER
...
{
    /** #CUSTOM# Передан доп параметр $arParams */
	function GetTree($dir, $max_depth, $get_description = false,$arParams = [])
	{
		$arMap = GetTreeRecursive($dir, 0, $max_depth, $get_description,$arParams);
...
{
    /** #CUSTOM# Передан доп параметр $arParams */
	function GetTreeRecursive($PARENT_PATH, $level, $max_depth, $get_description = false, $arParams = [])
	{
        ...
        /** #CUSTOM# Если текущий уровень 2, ищем 3-й уровень по кастомному меню */
        if($level == 2 && $arParams['THIRD_LEVEL_MENU_TYPE']){
            $arrMenu[0] = $arParams['THIRD_LEVEL_MENU_TYPE'];
        }
		$map = array();
						...
						if ($full_path <> '')
						{
							...
							if ($FILE_ACCESS!="D" && (!isset($aMenu[3]["SEPARATOR"]) || $aMenu[3]["SEPARATOR"]!="Y"))
							{
								...
								if ($search_child)
								{
                                    ...
                                    /** #CUSTOM# передан $arParams */
									$ar["CHILDREN"] = GetTreeRecursive($_SERVER["DOCUMENT_ROOT"].$ar["SEARCH_PATH"], $level+1, $max_depth, $get_description,$arParams);
								}
...
if ($this->StartResultCache(false, $additionalCacheID))
{
    ...
    /** #CUSTOM# передан $arParams */
	$arResult["arMapStruct"] = GetTree($_SERVER["DOCUMENT_ROOT"].$lang_dir, $arParams["LEVEL"], $arParams["SHOW_DESCRIPTION"] == "Y",$arParams);
