sprint_editor.registerBlock('my_detail_card', function ($, $el, data, settings) {
    ...
    this.afterRender = function () {
        ...
        sendrequest({
            ...
            info_elements: data.info_elements,
            enabled_iblocks: enabled_iblocks
        });
    ...
    var sendrequest = function (requestParams, callback) {
        ...
        $.ajax({
            url: sprint_editor.getBlockWebPath('my_detail_card') + '/ajax.php',
            type: 'post',
            data: requestParams,
            dataType: 'json',
            success: function (result) {

              data.info_elements = result.elements;

                $jresult.html(
                    sprint_editor.renderTemplate('my_detail_card-select', result)
                );
                ...
                $elem.sortable({
                  ...
                  stop: function (event, ui) {
                    let elementList = $elem.children();
                    let arrUpdIds = [];

                    // Собираем идентификаторы
                    elementList.each(function () {
                      arrUpdIds.push($(this).attr('data-id'));
                    });

                    // Создаем объект с ключами из arrUpdIds
                    data.info_elements = arrUpdIds.reduce((acc, id, index) => {
                      const parentVal = data.info_elements.find(element => element.id == id);
                      if (parentVal) {
                        acc[index] = parentVal; // Присваиваем элемент по индексу из arrUpdIds
                      }
                      return acc;
                    }, {});
                  }
