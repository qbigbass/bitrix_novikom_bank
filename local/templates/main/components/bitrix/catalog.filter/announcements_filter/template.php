        ...
        <div class="d-lg-none w-100 w-md-50">
            <select class="form-select form-select--size-small js-select" id="select1" aria-label="Подсказка" onchange="setFilter(this)">
                <?foreach ($arResult["ITEMS"] as $item):?>
                    <option
                        <?if ((int)$arParams["PARAMS_TYPE_SELECTED"] === 0):?>selected<?endif;?>
                        value="#to-all"
                        data-clearFilter="Y"
                    ...
                    <?foreach ($item["LIST"] as $xmlId => $value):?>
                        <?if ((int)$xmlId > 0):?>
                            <option
                                <?if((int)$arParams["PARAMS_TYPE_SELECTED"] === (int)$xmlId):?>selected<?endif;?>
                                value=""
                                data-setFilter="Y"
                ...
                <?foreach ($arResult["ITEMS"] as $item):?>
                    <?if(!empty($item["LIST"])):?>
                        <li class="swiper-slide w-auto tabs-panel__list-item nav-item z-2">
                            <a
                                class="tabs-panel__list-item-link nav-link bg-transparent <?if((int)$arParams["PARAMS_TYPE_SELECTED"] === 0):?>active<?endif;?>"
                                aria-current="page"
                                href="#"
                                data-filter="<?=$APPLICATION->GetCurPage(false)?>"
                                data-clearFilter="Y"
                        ...
                        <?foreach ($item["LIST"] as $xmlId => $value):?>
                            <?if ((int)$xmlId > 0):?>
                                <li class="swiper-slide w-auto tabs-panel__list-item nav-item z-2">
                                    <a
                                        class="tabs-panel__list-item-link nav-link bg-transparent <?if((int)$arParams["PARAMS_TYPE_SELECTED"] === (int)$xmlId):?>active<?endif;?>"
                                        href="#"
                                        data-setFilter="Y"
                                        data-filter="<?=$APPLICATION->GetCurPage(false)?>?<?= $item["INPUT_NAME"] ?>=<?= $xmlId ?>&set_filter=Y"
                                    ><?= $value ?></a>
