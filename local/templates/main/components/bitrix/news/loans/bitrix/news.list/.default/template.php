...
?>
<?php
//pre($arResult['PROPERTIES']['FIRST_FIELD_LIST']['VALUE']);
?>
    ...
    <div class="row">
        <div class="col-12 position-relative z-1 d-flex flex-column align-items-start gap-5 gap-md-7">
            <? foreach ($arResult['ITEMS'] as $loan) {
                ...
                if ($loan['PROPERTIES']['SHORT_TERMS_LIST']['VALUE']) {
                    $shortTerms = array_map(function ($term) {
                        return unserialize(htmlspecialcharsback($term));
                    }, $loan['PROPERTIES']['SHORT_TERMS_LIST']['VALUE']);
                }
                ?>
                ...
                <div class="card-product-list overflow-hidden position-relative mh-100 h-auto bg-dark-10 w-100 py-6 py-sm-9 py-md-11 px-3 px-sm-4 px-md-6 pe-xxl-11">
                    <div class="card-product-list__inner d-flex flex-column flex-lg-row align-items-start h-100 gap-3 gap-md-6 gap-xxl-11">
                        ...
                        <div class="card-product-list__content flex-column d-flex align-items-start gap-6 gap-xxl-9 w-100">
                            ...
                            <div class="card-product-list__condition-list w-100 w-lg-auto d-flex justify-content-between justify-content-lg-start flex-column flex-sm-row flex-wrap row-gap-4 row-gap-sm-6 row-gap-lg-6 row-gap-xxl-6 gap-lg-12 gap-xxl-16">
                                <? foreach ($shortTerms as $term) { ?>
                                    <div class="card-product-list__condition d-flex flex-column gap-2 w-100 w-sm-50 w-lg-auto">
                                        <div class="d-inline-flex flex-nowrap align-items-baseline gap-1">
                                            <? if (!empty($term['SECOND'])) { ?>
                                                <span class="text-l fw-semibold"><?= $term['SECOND'] ?></span>
                                            <? } ?>
                                            <? if (!empty($term['THIRD'])) { ?>
                                                <span class="text-number-l fw-bold"><?= $term['THIRD'] ?></span>
                                            <? } ?>
                                        </div>
                                        <? if (!empty($term['FIRST'])) { ?>
                                            <span class="text-m dark-70"><?= $term['FIRST'] ?></span>
                                        <? } ?>
                                    </div>
                                <? } ?>
                            </div>
                            <div class="d-flex flex-column flex-sm-row align-items-center gap-5 gap-sm-6 w-100">
                                <? if ($loan['PROPERTIES']['BUTTON_LIST']['VALUE']) { ?>
                                    <a class="btn btn-tertiary btn-lg card-product-list__button w-100 w-sm-auto" href="#"><?= $loan['PROPERTIES']['BUTTON_TEXT_LIST']['VALUE'] ?></a>
                                <? } ?>
                                <a class="btn btn-link btn-lg d-inline-flex gap-2 align-items-center card-product-list__button-more" href="<?= $loan['DETAIL_PAGE_URL'] ?>">
                                    <span>Подробнее</span>
