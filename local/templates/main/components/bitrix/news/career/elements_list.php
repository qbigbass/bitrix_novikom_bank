...
<section class="section-career-list section-layout py-lg-11 border-top border-blue10">
    <div class="container d-flex flex-column row-gap-4 row-gap-md-6 row-gap-lg-7">
        ...
        if (!empty($_GET)) {
            foreach ($_GET as $type => $value) {
                ...
                match ($type) {
                    'region', 'specialization' => $GLOBALS[$arParams["FILTER_NAME"]]['PROPERTY_' . strtoupper($type) . '_VALUE'] = $value,
                    'text' => $GLOBALS[$arParams["FILTER_NAME"]]['%NAME'] = $value,
                    default => null,
                };
            }
        }
    ...
    <picture class="pattern-bg d-none d-md-block pattern-bg--position-sm-top">
