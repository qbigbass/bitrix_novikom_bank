        ...
        <? if ($arWorkflowVariables): ?>
            <div class="adm-detail-content-item-block">
                <table width="100%" border="0" class="internal">
                    <tr class="heading">
                        <td><?= GetMessage("NOVIKOM_SETTINGS_SETVARIABLE_NAME") ?></td>
                        <td><?= GetMessage("NOVIKOM_SETTINGS_SETVARIABLE_ID") ?></td>
                        <td><?= GetMessage("NOVIKOM_SETTINGS_SETVARIABLE_VALUE_FROM") ?></td>
                    </tr>
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="variable_value_from[<?= $key ?>]">
                                    <option
                                        value=""><?= GetMessage("NOVIKOM_SETTINGS_SETVARIABLE_NOT_SELECTED") ?></option>
                                    <? foreach ($arUserFields as $arField): ?>
                                        <option value="<?= $arField["FIELD_NAME"] ?>"
                                            <? if ($arCurrentValues["variable_value_from"][$key] == $arField["FIELD_NAME"]): ?>
                                                selected="selected"
                                            <? endif ?>
                                        ><?= $arField["EDIT_FORM_LABEL"] ?> (<?= $arField["FIELD_NAME"] ?>)
                                        </option>
                                    <? endforeach ?>
                                </select>
                            </td>
